package com.augusta.dev.personalize.utliz;

/**
 * Created by Shanmugavel on 10/28/2016.
 */

public class Constants {
    public final static String IS_FIRST = "isFirst";
    public final static String MODE_TYPE = "modeType";
    public final static String IS_SELECT = "isSelect";
    public final static String CALL = "call";
    public final static String MUSIC = "music";
    public final static String ALARM = "alarm";
    public final static String MODES = "modes";
    public final static String LOCATION_MODES = "LOCATION_MODES";

    public final static String ENABLE_NOTIFICATION = "enable_notification";
    public final static String ENABLE_PERSONALIZE = "enable_personalize";
    public final static String ENABLE_LOC_MODE = "enable_loc_mode";
    public final static String ENABLE_ROUSE_UP = "enable_rouse_up";
    public final static String BACKGROUND_LOC_UPDATE = "background_loc_update";

    public final static String TIME = "time";
    public final static String SETTINGS = "settings";
    public final static String MODE = "mode";

    public final static String ADDRESS = "address";
    public final static String LATITUDE = "latitude";
    public final static String LONGITUDE = "longitude";
    public final static int NOTIFICATION_ID = 1001;

    public final static int ONE_DAY = 24 * 60 * 60 * 1000;
    public final static String ONLISTUPDATE = "onListUpdate";

    public final static String SONG_ID = "song_id";
    public final static String SONG_NAME = "song_name";
    public final static String SONG_PATH = "song_path";
    public final static String SONG_SELECT_LIST = "song_select_list";
    public final static String REALM_NAME = "realm_name";

    public static final int MULTIPLE_PERMISSIONS = 10;
}
